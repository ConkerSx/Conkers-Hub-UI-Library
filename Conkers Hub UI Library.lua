...
-- ==========================================================================
-- KEY SYSTEM — tela bloqueante exibida antes da janela principal.
-- Retorna true se a key foi validada, false se o usuário fechou sem validar.
-- ==========================================================================
local function RunKeySystem(config)
    local HttpService = game:GetService("HttpService")

    local checkUrl  = config.KeySystemURL or ""
    local discordUrl = config.KeySystemDiscord or "discord.gg/SEULINK"
    local RADIUS = 10
    ...
    local LANG = {
        PT = {
            ...
            btnSubmit   = "Verificar Key",
            btnCopy     = "Copiar link do Discord",
            copied      = "✓ Copiado!",
            notice      = "Você não precisa passar por nenhum encurtador. Apenas entre no servidor do Discord e gere sua key com o bot — depois cole ela aqui.",
            checking    = "Verificando...",
            errInvalid  = "Key inválida, expirada ou pertence a outro usuário.",
            errEmpty    = "Digite uma key antes de verificar.",
            success     = "Key verificada com sucesso!",
            whyTitle    = "Por que adicionar esse Key System?",
            whyReason1  = "1. Para garantir que ninguém simplesmente entre no Discord, copie o script e coloque nos comentários dos vídeos.",
            whyReason2  = "2. Para vocês ficarem informados sobre o script hub: se está funcionando, se saiu update, avisos, etc.",
        },
        EN = {
            tabKey      = "Key System",
            tabWhy      = "Why?",
            placeholder = "Paste your key here...",
            btnSubmit   = "Verify Key",
            btnCopy     = "Copy Discord link",
            copied      = "✓ Copied!",
            notice      = "You don't need to go through any link shortener. Just join the Discord server and generate your key with the bot — then paste it here.",
            checking    = "Verifying...",
            errInvalid  = "Invalid or expired key, or it belongs to another user.",
            errEmpty    = "Enter a key before verifying.",
            success     = "Key verified successfully!",
            whyTitle    = "Why was this Key System added?",
            whyReason1  = "1. To make sure no one just joins the Discord, copies the script, and pastes it in video comments.",
            whyReason2  = "2. To keep you informed about the script hub: if it's working, updates, notices, etc.",
        },
    }
    local lang = LANG.PT
    ...
    Create("Frame", {
        ...
        BackgroundColor3 = Theme.Background, BorderSizePixel = 0, ZIndex = 2, Parent = TopBar,
    })
    ...
    local TabsLy = ListLayout(TabsRow, Enum.FillDirection.Horizontal, 0)
    TabsLy.HorizontalAlignment = Enum.HorizontalAlignment.Center

    local TabKeyBtn = Create("TextButton", {
    ...
    local TabWhyBtn = Create("TextButton", {
        Size = UDim2.new(0.5, 0, 1, 0),
    ...
    local BodyDivider = Create("Frame", {
    ...
    Padding(PageKey, 14, 14, 16, 16)
    local PKLy = ListLayout(PageKey, Enum.FillDirection.Vertical, 10)

    local NoticeLbl = Create("TextLabel", {
    ...
    KeyInput.FocusGained:Connect(function() Tween(KeyBoxStroke, 0.15, {Color = Theme.Accent}) end)
    KeyInput.FocusLost:Connect(function() Tween(KeyBoxStroke, 0.15, {Color = Theme.Stroke}) end)
    ...
    Corner(CopyBtn, 8)
    Stroke(CopyBtn, Theme.StrokeAccent, 1)

    SubmitBtn.MouseEnter:Connect(function() Tween(SubmitBtn, 0.12, {BackgroundColor3 = Theme.AccentLight}) end)
    SubmitBtn.MouseLeave:Connect(function() Tween(SubmitBtn, 0.12, {BackgroundColor3 = Theme.Accent}) end)
    CopyBtn.MouseEnter:Connect(function() Tween(CopyBtn, 0.12, {BackgroundColor3 = Theme.ElementHover}) end)
    CopyBtn.MouseLeave:Connect(function() Tween(CopyBtn, 0.12, {BackgroundColor3 = Theme.Element}) end)

    CopyBtn.MouseButton1Click:Connect(function()
        local copied = false
        if type(setclipboard) == "function" then pcall(setclipboard, discordUrl); copied = true end
        if not copied and type(toclipboard) == "function" then pcall(toclipboard, discordUrl); copied = true end
        pcall(function()
            local gs = game:GetService("GuiService")
            if gs then gs.Clipboard = discordUrl end
        end)
        CopyBtn.Text = lang.copied
        task.delay(1.4, function()
            if CopyBtn and CopyBtn.Parent then CopyBtn.Text = lang.btnCopy end
        end)
    end)

    -- ── Página: Por que? ─────────────────────────────────────────────
    local PageWhy = Create("Frame", {
        Position = UDim2.new(0, 0, 0, 86),
        Size = UDim2.new(1, 0, 1, -86),
        BackgroundTransparency = 1,
        Visible = false,
    ...
    Padding(PageWhy, 14, 14, 16, 16)
    local PWLy = ListLayout(PageWhy, Enum.FillDirection.Vertical, 8)

    local WhyTitle = Create("TextLabel", {
        Size = UDim2.new(1, 0, 0, 0),
        AutomaticSize = Enum.AutomaticSize.Y,
        BackgroundTransparency = 1,
        Text = lang.whyTitle,
    ...
    local WhyR1 = Create("TextLabel", {
        Size = UDim2.new(1, 0, 0, 0),
        AutomaticSize = Enum.AutomaticSize.Y,
        BackgroundTransparency = 1,
        Text = lang.whyReason1,
        TextColor3 = Theme.TextSecondary,
        TextSize = 12,
        Font = Enum.Font.Gotham,
        TextWrapped = true,
    ...
    local WhyR2 = Create("TextLabel", {
        Size = UDim2.new(1, 0, 0, 0),
        AutomaticSize = Enum.AutomaticSize.Y,
        BackgroundTransparency = 1,
        Text = lang.whyReason2,
        TextColor3 = Theme.TextSecondary,
        TextSize = 12,
        Font = Enum.Font.Gotham,
        TextWrapped = true,
    ...
    local function ShowTab(which)
        if which == "key" then
            ...
            PageWhy.Visible = false
            TabKeyBtn.BackgroundColor3 = Theme.Accent
            TabKeyBtn.TextColor3 = Theme.TextPrimary
            TabWhyBtn.BackgroundColor3 = Theme.TabInactive
            TabWhyBtn.TextColor3 = Theme.TextSecondary
        else
            PageKey.Visible = false
            PageWhy.Visible = true
            TabWhyBtn.BackgroundColor3 = Theme.Accent
            TabWhyBtn.TextColor3 = Theme.TextPrimary
            TabKeyBtn.BackgroundColor3 = Theme.TabInactive
            TabKeyBtn.TextColor3 = Theme.TextSecondary
        end
    end
    TabKeyBtn.MouseButton1Click:Connect(function() ShowTab("key") end)
    TabWhyBtn.MouseButton1Click:Connect(function() ShowTab("why") end)

    -- ── Troca de idioma ──────────────────────────────────────────────
    local function ApplyLang()
        TabKeyBtn.Text = lang.tabKey
        TabWhyBtn.Text = lang.tabWhy
        KeyInput.PlaceholderText = lang.placeholder
        SubmitBtn.Text = lang.btnSubmit
        CopyBtn.Text = lang.btnCopy
        NoticeLbl.Text = lang.notice
        WhyTitle.Text = lang.whyTitle
        WhyR1.Text = lang.whyReason1
        WhyR2.Text = lang.whyReason2
        if ErrorLbl.Text ~= "" then ErrorLbl.Text = "" end
    end
    local usingPT = true
    LangBtn.MouseButton1Click:Connect(function()
        usingPT = not usingPT
        lang = usingPT and LANG.PT or LANG.EN
        LangBtn.Text = usingPT and "PT 🇧🇷" or "EN 🇺🇸"
        ApplyLang()
    end)

    -- ── Validação da key (lógica fornecida) ──────────────────────────
    local checking = false
    SubmitBtn.MouseButton1Click:Connect(function()
        if checking then return end
        local keyDigitada = KeyInput.Text
        if not keyDigitada or keyDigitada:gsub("%s+", "") == "" then
            ErrorLbl.Text = lang.errEmpty
            return
        end
        if checkUrl == "" then
            ErrorLbl.Text = "KeySystemURL não configurada."
            return
        end

        checking = true
        ErrorLbl.Text = ""
        local original = SubmitBtn.Text
        SubmitBtn.Text = lang.checking

        local userId = LocalPlayer.UserId
        local apiUrl = checkUrl .. "?key=" .. HttpService:UrlEncode(keyDigitada) .. "&user=" .. tostring(userId)
        ...
        if sucesso and resposta == "Liberado" then
            ErrorLbl.TextColor3 = Color3.fromRGB(110, 230, 140)
            ErrorLbl.Text = lang.success
            approved = true
            ...
            ErrorLbl.Text = lang.errInvalid
        end
    end)

    -- ── Loop bloqueante: espera até validar ou a Gui ser destruída ───
...
function ConkersHub:CreateWindow(config)
    ...
    -- KEY SYSTEM
    -- ======================================================================
    -- config.KeySystem = true/false                → liga/desliga localmente
    -- config.KeySystemURL ("...?key=&user=")        → endpoint /check da API
    -- config.KeySystemRemoteToggle = "https://..."  → gist/raw que retorna
    --                                                  "true" ou "false" puro.
    --                                                  Se "false", o KS é
    --                                                  ignorado pra TODOS,
    --                                                  mesmo com KeySystem=true.
    -- config.KeySystemDiscord = "discord.gg/..."     → link mostrado/copiado
    if config.KeySystem then
        local ksRemoteEnabled = true
        if config.KeySystemRemoteToggle and config.KeySystemRemoteToggle ~= "" then
            local ok, body = pcall(function()
                return game:HttpGet(config.KeySystemRemoteToggle)
            end)
            if ok and body then
                local clean = body:gsub("%s+", ""):lower()
                if clean == "false" then ksRemoteEnabled = false end
            end
        end

        if ksRemoteEnabled then
            local approved = RunKeySystem(config)
            if not approved then
                -- O jogador fechou a UI sem validar: aborta a criação da janela.
                return nil
            end
        end
    end
    ...
    -- Título do Dialog (visível apenas quando Title é fornecido)
    ...
    RegisterTheme(DialogTitle, "TextColor3", "TextPrimary")
    RegisterThemeAll(DialogTitle, "TextColor3", "TextPrimary")
    ...
    local function OpenDialog(cfg)
        local titleText = cfg.GuiTitle or ""
        DialogTitle.Text = titleText
        DialogTitle.Visible = (titleText ~= "")
    ...
    local function MakeComponents(container)
        ...
        function obj:AddDropdown(cfg)
            ...
            -- outsideConn: listener global descartável para fechar ao clicar fora
            local outsideConn = nil
            local function disconnectOutside()
                if outsideConn then outsideConn:Disconnect(); outsideConn = nil end
            end
            local function connectOutside(popupFrame, triggerFrame, closeFn)
                disconnectOutside()
                outsideConn = UserInputService.InputBegan:Connect(function(input)
                    if input.UserInputType ~= Enum.UserInputType.MouseButton1
                    and input.UserInputType ~= Enum.UserInputType.Touch then return end
                    if not listOpen then disconnectOutside(); return end
                    local mp = input.Position
                    local function inside(f)
                        if not f or not f.Visible then return false end
                        local a, sz = f.AbsolutePosition, f.AbsoluteSize
                        return mp.X >= a.X and mp.X <= a.X + sz.X
                           and mp.Y >= a.Y and mp.Y <= a.Y + sz.Y
                    end
                    if not inside(popupFrame) and not inside(triggerFrame) then
                        closeFn()
                    end
                end)
            end
            ...
            local function OpenList()
                ...
                local listW = math.min(cs.X - 16, 300)
                local posX  = math.clamp(ca.X + cs.X / 2 - listW / 2, 4, vp.X - listW - 4)
                local posY  = math.clamp(ca.Y + cs.Y / 2 - listH / 2 + 35, 4, vp.Y - listH - 4)
                List.Parent = Overlay
                ...
                connectOutside(List, Container, CloseList)
        ...
        function obj:AddMultiDropdown(cfg)
            ...
            local listOpen = false
            local outsideConnM = nil
            local function disconnectOutsideM()
                if outsideConnM then outsideConnM:Disconnect(); outsideConnM = nil end
            end
            local function connectOutsideM(popupFrame, triggerFrame, closeFn)
                disconnectOutsideM()
                outsideConnM = UserInputService.InputBegan:Connect(function(input)
                    if input.UserInputType ~= Enum.UserInputType.MouseButton1
                    and input.UserInputType ~= Enum.UserInputType.Touch then return end
                    if not listOpen then disconnectOutsideM(); return end
                    local mp = input.Position
                    local function inside(f)
                        if not f or not f.Visible then return false end
                        local a, sz = f.AbsolutePosition, f.AbsoluteSize
            ...
            local function OpenMulti()
                ...
                connectOutsideM(List, Container, CloseMulti)
                Tween(List, 0.22, {Size = UDim2.new(0, listW, 0, listH)},
                    Enum.EasingStyle.Quart, Enum.EasingDirection.Out)
                Tween(ArrowLbl, 0.2, {Rotation = 180})
            end
        ...
        function obj:AddColorPicker(cfg)
            ...
            local outsideConnP = nil
            local function disconnectOutsideP()
                if outsideConnP then outsideConnP:Disconnect(); outsideConnP = nil end
            end
            local function connectOutsideP(popupFrame, triggerFrame, closeFn)
                disconnectOutsideP()
                outsideConnP = UserInputService.InputBegan:Connect(function(input)
                    if input.UserInputType ~= Enum.UserInputType.MouseButton1
                    and input.UserInputType ~= Enum.UserInputType.Touch then return end
                    if not popOpen then disconnectOutsideP(); return end
                    if centralDragActive then return end
                    local mp = input.Position
                    local function inside(f)
                        if not f or not f.Visible then return false end
            ...
            -- Snapshot de transparências do Popup (salvo uma vez após criar)
            -- Usado para restaurar após o fade de saída.
            local popupTransSnap = nil
            local function buildPopupSnap()
                popupTransSnap = {}
                popupTransSnap[Popup] = {bg = Popup.BackgroundTransparency}
                for _, child in ipairs(Popup:GetDescendants()) do
                    if child:IsA("UIStroke") then
                        popupTransSnap[child] = {stroke = child.Transparency}
                    elseif child:IsA("ImageLabel") or child:IsA("ImageButton") then
                        popupTransSnap[child] = {bg = child.BackgroundTransparency, img = child.ImageTransparency}
                    elseif child:IsA("Frame") or child:IsA("ScrollingFrame") then
                        popupTransSnap[child] = {bg = child.BackgroundTransparency}
                    end
                end
            end
            local function restorePopupSnap()
                if not popupTransSnap then return end
                for inst, vals in pairs(popupTransSnap) do
                    if inst and inst.Parent then
                        pcall(function()
                            if vals.bg  ~= nil then inst.BackgroundTransparency = vals.bg end
                            if vals.img ~= nil then inst.ImageTransparency = vals.img end
                            if vals.stroke ~= nil then inst.Transparency = vals.stroke end
                        end)
                    end
                end
            end
            ...
            local function ClosePopup()
                ...
                CentralOverlay.Visible = false
                disconnectOutsideP()
                -- Fade out limpo: anima apenas transparência, sem deformar tamanho
                local FADE_T = 0.15
                Tween(Popup, FADE_T, {BackgroundTransparency = 1},
                    Enum.EasingStyle.Linear, Enum.EasingDirection.Out)
                for _, child in ipairs(Popup:GetDescendants()) do
                    if child:IsA("Frame") or child:IsA("ScrollingFrame") then
                        pcall(function()
                            Tween(child, FADE_T, {BackgroundTransparency = 1},
                                Enum.EasingStyle.Linear, Enum.EasingDirection.Out)
                        end)
                    elseif child:IsA("UIStroke") then
                        pcall(function()
                            Tween(child, FADE_T, {Transparency = 1},
                                Enum.EasingStyle.Linear, Enum.EasingDirection.Out)
                        end)
                    elseif child:IsA("ImageLabel") or child:IsA("ImageButton") then
                        pcall(function()
                            Tween(child, FADE_T, {BackgroundTransparency = 1, ImageTransparency = 1},
                                Enum.EasingStyle.Linear, Enum.EasingDirection.Out)
                        end)
                    end
                end
                task.delay(FADE_T + 0.02, function()
                    if not popOpen and Popup then
                        Popup.Visible = false
                        restorePopupSnap()  -- restaura valores originais prontos para próxima abertura
                    end
                end)
            end
            ...
            CBtn.MouseButton1Click:Connect(function()
                ...
                local ca  = ContentArea.AbsolutePosition
                local cs  = ContentArea.AbsoluteSize
                popupW = math.min(cs.X - 16, 280)
                -- Centralizado na área de componentes (ContentArea)
                local posX = math.clamp(ca.X + cs.X / 2 - popupW / 2, 4, vp.X - popupW - 4)
                local posY = math.clamp(ca.Y + cs.Y / 2 - POPUP_H / 2 + 35, 4, vp.Y - POPUP_H - 4)
                ...
                connectOutsideP(Popup, Container, ClosePopup)
            ...
            -- Aplica estado inicial e salva snapshot de transparências do Popup
            Apply()
            task.defer(buildPopupSnap)  -- defer: garante que todos os filhos já existem
        ...
        -- Label
        -- AddLabel: componente com Title (negrito) + Text (descrição), com frame de fundo.
        function obj:AddLabel(cfg)
            ...
            Corner(Frame, 6)
            local FS = Stroke(Frame, Theme.Stroke, 1)
            RegisterTheme(Frame, "BackgroundColor3", "Element")
            RegisterThemeAll(Frame, "BackgroundColor3", "Element")
            RegisterTheme(FS, "Color", "Stroke")
            RegisterThemeAll(FS, "Color", "Stroke")
            Padding(Frame, 7, 7, 8, 8)

            local Inner = Create("Frame", {
                Size = UDim2.new(1, 0, 0, 0), AutomaticSize = Enum.AutomaticSize.Y,
                BackgroundTransparency = 1, ZIndex = 4, Parent = Frame,
            })
            local ILy = ListLayout(Inner, Enum.FillDirection.Vertical, 3)
            ...
            local TitleLbl = Create("TextLabel", {
                ...
                Font = Enum.Font.GothamBold,
                TextXAlignment = Enum.TextXAlignment.Left, TextWrapped = true,
                Visible = (title ~= ""), ZIndex = 5, Parent = Inner,
            })
            ...
            local TextLbl = Create("TextLabel", {
                ...
                Font = Enum.Font.Gotham,
                TextXAlignment = Enum.TextXAlignment.Left, TextWrapped = true,
                Visible = (text ~= ""), ZIndex = 5, Parent = Inner,
            })
            RegisterTheme(TextLbl, "TextColor3", "TextSecondary")
            RegisterThemeAll(TextLbl, "TextColor3", "TextSecondary")
        ...
        -- Dialog
        -- AddDialog: Text = nome do botão | GuiTitle = título no popup | GuiText = texto no popup
        function obj:AddDialog(cfg)
            cfg = cfg or {}
            local btnText  = cfg.Text   or cfg.Name or "Abrir Dialog"
            local guiTitle = cfg.GuiTitle or ""
            ...
            Create("TextLabel", {AnchorPoint = Vector2.new(0, 0.5), Position = UDim2.new(0, 28, 0.5, 0),
                Size = UDim2.new(1, -36, 1, 0), BackgroundTransparency = 1, Text = btnText,
            ...
            Btn.MouseButton1Click:Connect(function()
                OpenDialog({GuiTitle = guiTitle, GuiText = guiText, YesBtn = yesBtn, NoBtn = noBtn, Callback = callback})
            end)
        end
        ...
        function obj:AddSocials(cfg)
            ...
            local PLATFORM_META = {
                Discord    = {label = "Discord",   icon = "💬", color = Color3.fromRGB(88, 101, 242)},
                Youtube    = {label = "YouTube",   icon = "▶",  color = Color3.fromRGB(255, 50,  50)},
                TikTok     = {label = "TikTok",    icon = "♪",  color = Color3.fromRGB(255, 255, 255)},
                Linktree   = {label = "Linktree",  icon = "🌿", color = Color3.fromRGB(67,  198, 120)},
                ScriptBlox = {label = "ScriptBlox",icon = "📜", color = Color3.fromRGB(255, 165, 0)},
            }
            ...
            -- Wrapper externo (lista vertical de cards)
            ...
            local WLy = ListLayout(Wrapper, Enum.FillDirection.Vertical, 6)
            WLy.HorizontalAlignment = Enum.HorizontalAlignment.Center
            ...
            for idx, entry in ipairs(active) do
                ...
                Corner(Card, 10)
                local CS = Stroke(Card, Theme.Stroke, 1)
                RegisterTheme(Card, "BackgroundColor3", "Element")
                RegisterThemeAll(Card, "BackgroundColor3", "Element")
                RegisterTheme(CS, "Color", "Stroke")
                RegisterThemeAll(CS, "Color", "Stroke")

                -- Avatar / ícone (44x44, canto arredondado, cor da plataforma)
                ...
                -- Brilho sutil no avatar (overlay branco 80% transparente)
                local AvatarShine = Create("Frame", {
                    Size = UDim2.new(1, 0, 0.5, 0),
                ...
                -- ── Bloco de texto (nome + @user + link) ──────────────────
                local Info = Create("Frame", {
                    AnchorPoint = Vector2.new(0, 0.5),
                    Position = UDim2.new(0, 62, 0.5, 0),
                    Size = UDim2.new(1, -166, 1, -14),
                    BackgroundTransparency = 1,
                    ZIndex = 5,
                    Parent = Card,
                })
                local ILy = ListLayout(Info, Enum.FillDirection.Vertical, 2)
                ILy.VerticalAlignment = Enum.VerticalAlignment.Center
                ...
                -- ── Coluna direita: badge plataforma + botão Copy ──────────
                local RightCol = Create("Frame", {
                    AnchorPoint = Vector2.new(1, 0.5),
                    Position = UDim2.new(1, -8, 0.5, 0),
                    Size = UDim2.new(0, 88, 1, -14),
                    BackgroundTransparency = 1,
                    ZIndex = 5,
                    Parent = Card,
                })
                local RLy = ListLayout(RightCol, Enum.FillDirection.Vertical, 5)
                RLy.HorizontalAlignment = Enum.HorizontalAlignment.Right
                RLy.VerticalAlignment   = Enum.VerticalAlignment.Center

                -- Badge com nome da plataforma (ex: "Discord")
                ...
                -- Botão Copy — ZIndex alto pra ficar acima do CardHover
                local CopyBtn = Create("TextButton", {
                    Size = UDim2.new(1, 0, 0, 24),
                    BackgroundColor3 = accent,
                ...
                -- Hover direto no Frame do card (sem TextButton overlay que bloqueia cliques)
                ...
                CopyBtn.MouseEnter:Connect(function()
                    Tween(CopyBtn, 0.12, {BackgroundTransparency = 0})
                end)
                CopyBtn.MouseLeave:Connect(function()
                    Tween(CopyBtn, 0.12, {BackgroundTransparency = 0.1})
                end)
                CopyBtn.MouseButton1Down:Connect(function()
                    Tween(CopyBtn, 0.08, {BackgroundTransparency = 0.3})
                end)

                -- Copy com fallback para múltiplos executores
                CopyBtn.MouseButton1Click:Connect(function()
                    local copied = false
                    if link ~= "" then
                        if type(setclipboard) == "function" then
                            pcall(setclipboard, link); copied = true
                        end
                        if not copied and type(syn) == "table" and syn.clipboard and type(syn.clipboard.set) == "function" then
                            pcall(syn.clipboard.set, link); copied = true
                        end
                        if not copied and type(toclipboard) == "function" then
                            pcall(toclipboard, link); copied = true
                        end
                        if not copied and type(writefile) == "function" then
                    ...
                    Tween(CopyBtn, 0.1, {BackgroundTransparency = 0})
                    task.delay(1.5, function()
                        if CopyBtn and CopyBtn.Parent then
                            CopyBtn.Text = "Copiar link"
                            Tween(CopyBtn, 0.2, {BackgroundTransparency = 0.1})
                        end
                    end)
                end)
            end
        ...
        obj._N = N -- expõe o contador de ordem para uso externo (ex: Tab:AddSection)
        return obj
    end -- MakeComponents
    ...
    function Window:AddTab(tabConfig)
        ...
        Tab.Label = Tab.AddLabel       -- componente com Title + Text
        ...
        function Tab:AddSection(sectionConfig)
            ...
            local secName = sectionConfig.Name or sectionConfig.Text or "Section"

            local Lbl = Create("TextLabel", {
                Size = UDim2.new(1, 0, 0, 0), AutomaticSize = Enum.AutomaticSize.Y,
                BackgroundTransparency = 1, Text = secName,
                TextColor3 = sectionConfig.Color or Theme.TextSecondary,
                TextSize = sectionConfig.TextSize or 12,
                Font = Enum.Font.Gotham, TextXAlignment = Enum.TextXAlignment.Left, TextWrapped = true,
                LayoutOrder = Tab._N(), ZIndex = 3, Parent = TabPage,
            })
            Padding(Lbl, 2, 2, 6, 6)
            RegisterTheme(Lbl, "TextColor3", "TextSecondary")
            RegisterThemeAll(Lbl, "TextColor3", "TextSecondary")

            local S = {}
            function S:Set(t) Lbl.Text = tostring(t) end
            function S:SetColor(c) Lbl.TextColor3 = c end
            return S
        end

        -- Aliases que dependem de AddSection já estar definido
        Tab.Section = Tab.AddSection
        Tab.AddDivider2 = Tab.AddSection -- nome alternativo, evita colidir com AddSeparator
